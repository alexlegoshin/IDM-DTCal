...
def _measure_branch(dmm: DMM, src: CurrentSource,
    ...
    выходное напряжение датчика; оно биполярное (по умолчанию -4..+4 В), на
    обратной ветви вольтметр читает отрицательные значения — это норма, а не
    ошибка знака. Диапазоном вольтметра здесь не управляем: шкала узкая и
    настраивается один раз при инициализации прибора (аппаратный
    автодиапазон, см. instruments.py). sign используется для записи знака в I_set.
