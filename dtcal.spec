...
"""
PyInstaller-спецификация сборки DTCal в portable-приложение (onedir).

Сборка:
    pyinstaller dtcal.spec --noconfirm

Результат: dist/DTCal/DTCal.exe (+ папка _internal). Копируется целиком
...
приложение намеренно не дробится.
"""
